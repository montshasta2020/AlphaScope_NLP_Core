...
model = KeyedVectors.load_word2vec_format('data/chinese_model_format_v1.bin', binary=False)
...
def guoxue():
    global model
    res = model.most_similar(positive = [u'国学', u'文化'],topn = 200)
    words = [t[0] for t in res]
    return render_template('guoxue.html', words_out = words)

@app.route('/guoxue/', methods=['POST'])
def guoxue_post():
    global model

    if request.method == 'POST':
        text = request.form['topics_sel']
        #text = request.form['query_input']
        print text

        res = model.most_similar(text,topn = 200)
        words = [t[0] for t in res]
        return render_template('listtopic.html', title = text, words_out = words)


@app.route('/zhexue/')
def zhexue():
    return render_template('zhexue.html')
